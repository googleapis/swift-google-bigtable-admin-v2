...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-bigtable-admin-v2.git --from 0.1.0-preview
```

Then add `GoogleCloudBigtableAdminV2` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudBigtableAdminV2 <target-name> --package swift-google-bigtable-admin-v2
```
