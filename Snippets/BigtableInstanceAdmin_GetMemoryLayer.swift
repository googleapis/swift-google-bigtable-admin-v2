...
func sample(
  ...
  let response = try await client.getMemoryLayer(
    request: GetMemoryLayerRequest()
      .with {
        $0.name = "projects/\(projectId)/instances/\(instanceId)/clusters/\(clusterId)/memoryLayer"
      }
  )
